<?php 
	...
	function update_notices($id,$date, $notice){
		$con = getConnection();
		$sql = "update notices set date='{$date}', notice='{$notice}' where id='{$id}'";
		if(mysqli_query($con, $sql)){
			return true;
		}else{
			return false;
	...
	function Add_buyerpost($date, $name, $details, ){		
		$con = getConnection();
		$sql = "insert into buyerpost (id, date, name, details) values('','{$date}','{$name}','{$details}')";
		if(mysqli_query($con, $sql)){
			return true;
		}else{
			return false;
